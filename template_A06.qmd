...
```{r}
round_points_per_game <- sapply(list_points_per_game, function(i){round(i,2)})
...
round_mat <- apply(mat_rand, MARGIN= 1:2, my_func)
...
```{r}
months_matrix <- max_temps[,3:14]
average_temps <- apply(months_matrix, 2, mean)
...
```{r}
co_mat <- subset(max_temps, state == "CO")

oh_mat <- subset(max_temps, state == "OH")

va_mat <- subset(max_temps, state == "VA")
